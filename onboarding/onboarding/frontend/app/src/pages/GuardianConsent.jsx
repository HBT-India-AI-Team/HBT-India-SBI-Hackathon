...
export default function GuardianConsent() {
  ...
  const requestLink = async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.detail || 'Could not create guardian link.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const continueAsGuardian = async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.detail || 'Could not open guardian session.');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <PhoneScreen title="Guardian consent needed">
      <div className="flex flex-col items-center text-center gap-3 mb-6">
        <div className="w-16 h-16 rounded-full bg-primary-container/15 flex items-center justify-center text-3xl">
          🧑‍🤝‍🧑
        </div>
        <h2 className="font-heading font-bold text-lg text-primary">This account needs a guardian</h2>
        <p className="text-on-surface-variant text-[14px] max-w-xs">
          Since this is a minor account, we need a parent/guardian to confirm consent and verify their own mobile
          number before we continue.
        </p>
      </div>
      ...
      {!link ? (
        <div className="flex flex-col gap-3">
          <div>
            <label className="text-[12px] font-semibold text-on-surface-variant">Guardian mobile number</label>
            <input
              value={mobile}
          ...
          <div>
            <label className="text-[12px] font-semibold text-on-surface-variant">Relationship</label>
            <select
              value={relationship}
          ...
          <PrimaryButton onClick={requestLink} disabled={busy || !mobile}>
            {busy ? 'Generating link…' : 'Send consent link to guardian'}
          </PrimaryButton>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          <div className="bg-surface-container-low rounded-xl p-4">
            <p className="text-[12.5px] text-on-surface-variant mb-1">Guardian link generated (mock-sent):</p>
            <p className="text-[12px] font-mono break-all text-primary">{link.link}</p>
          </div>
          {error && <p className="text-error text-[12.5px]">{error}</p>}
          <PrimaryButton onClick={continueAsGuardian} disabled={busy}>
            {busy ? 'Opening…' : 'Continue as guardian now (demo)'}
          </PrimaryButton>
          <p className="text-[11.5px] text-on-surface-variant text-center">
            In production the guardian opens this link on their own device — this button simulates that for the
            demo so the flow can be tested end-to-end in one browser.
          </p>
        </div>
      )}
    </PhoneScreen>
