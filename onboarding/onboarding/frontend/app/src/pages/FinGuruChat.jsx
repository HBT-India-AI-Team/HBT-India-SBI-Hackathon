...
export default function FinGuruChat() {
  ...
  const [liveSpeaking, setLiveSpeaking] = useState(false); // server-echo/FinGuru audio OR Ollama-TTS audio currently playing
  ...
  const runStreamingOllama = async (text) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: newMsgId(), direction: 'inbound', text },
    ...
    try {
      ...
      if (result?.ttsFallbackNeeded) {
        ...
        try {
          const blob = await synthesizeText(result.text || acc, languageRef.current);
          setPreparingId(null);
  ...
  // Live-call voice mode (VITE_VOICE_MODE=live): full-duplex WS to the voice
  // server. By default we use the WS only for streaming mic + the transcript,
  // then run that through FinGuru/Ollama (one-shot) and speak the answer via
  // the existing REST /synthesize (the server's own echo reply is
  // suppressed). If VITE_VOICE_SKIP_FINGURU=true and Ollama is configured as
  // the voice brain, LLM_STREAMING_ENABLED further decides whether that
  // Ollama reply is streamed sentence-by-sentence to /tts-ws or also answered
  // the one-shot way. Only with neither FinGuru nor Ollama configured do we
  // fall back to playing the server's own built-in echo reply.
  const voiceCall = useVoiceCall({
    // Suppress the server echo whenever we generate our own reply (FinGuru,
    // one-shot Ollama, or streaming Ollama); keep it only for pure echo mode.
    playServerReplies: SKIP_FINGURU_IN_VOICE && !OLLAMA_FALLBACK_ACTIVE,
    onSpeakingChange: setLiveSpeaking,
    language, // selected language -> sent on the live-call WS handshake
    onTranscript: (text, detectedLanguage) => {
      ...
      applyDetectedLanguage(detectedLanguage);
      ...
      if (SKIP_FINGURU_IN_VOICE && !OLLAMA_FALLBACK_ACTIVE) {
        // Pure echo: neither FinGuru nor Ollama configured -- just show what
        // was heard; the server speaks its own (placeholder) reply.
        setMessages((prev) => [...prev, { id: newMsgId(), direction: 'inbound', text: t }]);
        return;
      }
    ...
    onReplyText:
      SKIP_FINGURU_IN_VOICE && !OLLAMA_FALLBACK_ACTIVE
        ? (text) => {
            if (text?.trim()) setMessages((prev) => [...prev, { id: newMsgId(), direction: 'outbound', text: text.trim() }]);
          }
        : undefined,
    onError: (reason) => {
      setVoiceNote(
      ...
      // Prompt 7: reconnection exhausted (Prompt 4) or the socket never
      // opened at all -- surface the text fallback rather than leaving the
      // user stuck in a dead call. Both STT and TTS ride the same combined
      // WS here, so neither is usable until it reconnects.
      if (reason === 'reconnect_exhausted' || reason === 'ws_failed') {
  ...
  // across all three reply pipelines, so a new turn can start immediately.
  ...
  // transcript could sneak in as "not busy".
  const stopSpeakingNow = () => {
    ...
    setOllamaThinking(false);
    setVoiceStatus('idle');
    voiceCallRef.current?.stopSpeaking(); // server-audio path (echo mode) -- no-op if nothing playing
  ...
  const speakMessage = async (id, text) => {
    ...
    try {
      ...
      const blob = await synthesizeText(text, languageRef.current);
  ...
  const send = async (text, opts = {}) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: newMsgId(), direction: 'inbound', text: trimmed, viaFallback: !!opts.viaFallback },
    ]);
    setInput('');
    setLoading(true);
    try {
      const res = await askBrain(trimmed, history, {
        colloquial: colloquialRef.current,
        language: languageRef.current,
        voice: !!opts.speak,
  ...
  // language the user spoke. Only auto-switches to a language the picker offers
  // and only when it actually differs, to avoid needless churn. Returns the
  // applied code, or null if nothing changed.
  const applyDetectedLanguage = (detected) => {
    // Language is locked while a live call is active -- the call negotiated its
    // language at connect time and can't change mid-stream, so neither the user
    // (dropdown disabled) nor STT auto-detect should move it during the call.
    if (inCall) return null;
    const code = resolveLanguageCode(detected);
    if (!code || !ENABLED_LANGUAGES.includes(code) || code === languageRef.current) return null;
  ...
  const processVoiceJob = async ({ id, blob }) => {
    ...
    let transcript = '';
    try {
      const result = await transcribeAudio(blob, languageRef.current);
      transcript = (result?.text || '').trim();
      applyDetectedLanguage(result?.detected_language);
    ...
    updateMessageById(id, { text: transcript, status: 'processing', statusText: 'Getting a reply…' });
    ...
    try {
      const audioBlob = await synthesizeText(replyText, languageRef.current);
      const url = URL.createObjectURL(audioBlob);
      updateMessageById(replyId, { status: 'done', audioUrl: url, audioBlob, autoPlay: true });
  ...
  const languageSelect = (
    <select
      ...
      title={inCall ? 'Language is locked during a voice call' : 'Language for replies and voice'}
        ...
        {liveMode && inCall && (
          ...
          >
            ...
            >
              ...
              {voiceCall.lastLatencyMs != null && voiceCall.status === 'live' && (
                <span className="text-on-surface-variant font-normal shrink-0">
                  · {Math.round(voiceCall.lastLatencyMs / 1000)}s
                </span>
              )}
            </p>
            <button
