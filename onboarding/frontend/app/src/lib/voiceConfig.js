...
// If Ollama is also configured as the voice brain (VITE_USE_OLLAMA_IN_VOICE),
// it answers instead; if neither is configured, transcripts are shown with
// no spoken reply -- Sarvam's realtime call is STT-only, so there is no
// server-side echo to fall back to. Set VITE_VOICE_SKIP_FINGURU=true.
