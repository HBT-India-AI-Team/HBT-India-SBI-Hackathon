    ...
    <PhoneScreen
      ...
      footer={
        <div className="flex flex-col gap-3">
          ...
          <GhostButton onClick={() => navigate(-1)}>Not now</GhostButton>
        </div>
      }
    >
      <div className="flex flex-col items-center text-center gap-4 mt-6">
        ...
        <h2 className="font-heading font-bold text-xl text-primary">Your data, your control</h2>
        <p className="text-on-surface-variant text-[14.5px] max-w-xs">
          To open your account we'll need to verify your mobile number, PAN and a few documents. We only use this
          information for KYC and account opening, in line with RBI guidelines.
        </p>
      </div>
      <div className="mt-6 flex flex-col gap-3">
        {[
          ...
          <div key={title} className="flex items-start gap-3 bg-surface-container-low rounded-xl p-3.5">
            ...
            <div>
              <p className="font-semibold text-[13.5px] text-on-surface">{title}</p>
              <p className="text-[12.5px] text-on-surface-variant">{desc}</p>
            </div>
          </div>
        ))}
      </div>
