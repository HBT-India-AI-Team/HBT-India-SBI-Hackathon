...
export function AgentEditor({ agentId, agents, stages, capabilities, templates, onSaved }: AgentEditorProps) {
  ...
  const handleRefine = async () => {
    ...
    try {
      ...
      setRefineError(err instanceof ApiRequestError ? err.message : String(err))
    } finally {
      setRefining(false)
    }
  }
          ...
          <div className="shrink-0 border-b border-amber-200 bg-amber-50 px-4 py-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-amber-800">
                Generated by AI. Review the rules below, or describe what's wrong and let it correct itself.
              </span>
            </div>
            <div className="flex gap-2">
              <TextInput
                className="flex-1"
                placeholder="e.g. evidence should be a flat credit_score number, not nested"
                value={refineFeedback}
                onChange={(e) => setRefineFeedback(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && !refining && handleRefine()}
              />
              <Button size="sm" className="shrink-0" onClick={handleRefine} disabled={refining || !refineFeedback.trim()}>
                {refining ? 'Fixing…' : 'Fix with AI'}
              </Button>
            </div>
